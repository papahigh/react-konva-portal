...
describe('portal.tsx', () => {
  describe('content movement', () => {
    class App extends Component<PortalProps, PortalProps> {
      ...
      public render() {
        const { containerId, zIndex } = this.state;
        return (
          <Stage ref={ref => (this.stageRef = ref)}>
            ...
            <Layer id="layer-origin">
              <Portal containerId={containerId} zIndex={zIndex}>
                <Text name="x-content" />
              </Portal>
            </Layer>
  ...
  describe('content ordering', () => {
      ...
      public render() {
        ...
        return (
          <Stage ref={ref => (this.stageRef = ref)}>
            <Layer id="layer-slot" />
            <Layer id="layer-origin">
              <Portal containerId={a.containerId} zIndex={a.zIndex}>
                <Text name="A" />
              </Portal>
              <Portal containerId={b.containerId} zIndex={b.zIndex}>
                <Text name="B" />
              </Portal>
            </Layer>
          </Stage>
    ...
    function assertEmpty(stage: Konva.Stage, query: string) {
      const container = stage.find(query)[0];
      expect(container?.children.length).toEqual(0);
    }

    function assertOrder(stage: Konva.Stage, query: string, ...ordered: string[]) {
      const container = stage?.find(query)[0] as Konva.Container;
